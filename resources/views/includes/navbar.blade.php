@php
$materials = App\Models\Materials::orderBy('updated_at', 'desc')->whereColumn('new_stock', '<=', 'limit_stock')->take(3)->get();
    @endphp
    <nav class="fixed bg-white h-16 w-full flex flex-row justify-between gap-2 items-center md:px-20 px-4 z-50 shadow">
        ...
        @auth
        <div class="relative md:hidden flex flex-row items-center gap-4 justify-between text-white">
            <div class="relative">
                ...
                <div
                    ...
                    <div class="space-y-3 mt-3 text-gray-400">
                        @forelse ($materials as $material)
                        <p>{{ $material->name }} - {{ $material->new_stock }} pcs left</p>
                        @empty
                        <p>No Data</p>
                        @endforelse
                    </div>
            ...
            <ul id="menu"
                ...
                <li class="{{ request()->is('sop*') ? 'bg-primary text-white px-3 py-1 rounded-md' : '' }}"><a
                        href="/sop" class="hover:text-gray-400">SOP</a></li>
                <li class="{{ request()->is('instruksi-kerja*') ? 'bg-primary text-white px-3 py-1 rounded-md' : '' }}">
                    <a href="/instruksi-kerja" class="hover:text-gray-400">IK</a>
                </li>
