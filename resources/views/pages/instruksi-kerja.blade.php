...
<div>
    ...
    <div class="md:px-40 px-8 mb-20 -translate-y-16">
        <div class="grid md:grid-cols-6 grid-cols-2 gap-4 ik-container">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        document.getElementById('searchBtn').addEventListener('click', function() {
            ...
            xhr.open('GET', '/instruksi-kerja?keyword=' + encodeURIComponent(keyword), true);
            xhr.onreadystatechange = function() {
                if (xhr.readyState === XMLHttpRequest.DONE) {
                    if (xhr.status === 200) {
                        // Update konten hasil pencarian
                        document.querySelector('.ik-container').innerHTML = xhr.responseText;
                    } else {
