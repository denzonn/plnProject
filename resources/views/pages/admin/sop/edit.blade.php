...
@section('content')
    <div class="bg-white p-8 rounded-md text-gray-500">
        <div class="text-xl font-semibold">Edit SOP --- {{ $data->name }}</div>
        <div>
            <form action="{{ route('update-sop', $data->id) }}" enctype="multipart/form-data" method="POST">
                @csrf
                @method('PUT')
                ...
                <button type="submit" class="w-full rounded-md bg-primary mt-4 text-white py-2 text-lg">Edit SOP</button>
            </form>
        </div>
    </div>
@endsection
